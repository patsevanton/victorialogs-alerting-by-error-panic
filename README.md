...
Казалось бы, чем шире окно, тем надёжнее алерт: поставил `_time: 1h` — и ни одна ошибка не проскочит. Но `_time` — это не «посмотреть назад на час», а «каждое исполнение заново сканирует весь час». При `interval: 1m` правило исполняется каждую минуту, и на каждом запуске VictoriaLogs заново перечитывает последние 60 минут логов, из которых 59 минут были отсканированы минуту назад.
