...
Развернуть в managed Kubernetes связку, которая следит **за логами**: как только Go-приложение падает с `panic` или `log.Fatal`, а Nuxt-приложение отвечает 500-й или роняет необработанное исключение — в Telegram уходит алерт со ссылкой на приложение и количеством ошибок.
