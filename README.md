...
Классическая ситуация: Go-сервис падает с `panic: runtime error: invalid memory address or nil pointer dereference`, Nuxt-фронтенд отвечает 500-й или какая либо другая ошибка. Текст самой ошибки больше нигде не живёт — только в логах. Если нужно поймать проблему, нужен алертинг по логам: связка VictoriaLogs + vlagent + vmalert + Alertmanager следит за логами, и как только приложение роняет `panic` или `log.Fatal` — в Telegram уходит алерт.
