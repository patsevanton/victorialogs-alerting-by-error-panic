...
Классическая ситуация: Go-сервис падает с `panic: runtime error: invalid memory address or nil pointer dereference`, Nuxt-фронтенд отвечает 500-й, а узнаёте вы об этом только утром из отчёта — или вовсе случайно. Метрики тут помогают слабо: они проседают уже после факта, а pod к тому моменту успел перезапуститься, метрика `restart_count` выросла, и текст самой ошибки больше нигде не живёт — только в логах. Другого выхода нет: если нужно поймать проблему в момент её появления, а не по последствиям в метриках, смотреть приходится именно в логи. Поэтому нужен алертинг по логам: связка VictoriaLogs + vlagent + vmalert + Alertmanager следит за логами, и как только Go-приложение роняет `panic` или `log.Fatal`, а Nuxt-приложение отдаёт 500-ю или падает на необработанном исключении — в Telegram уходит алерт со ссылкой на приложение и количеством ошибок.
...
Файл правил решает это институционально. Каждое правило — явная строка в `vmalert-rules.yaml`, которую видно в git и которую можно отревьюить до попадания в ConfigMap. Видно интервал, запрос, порог, окно `for` — и можно проверить, что у каждого правила стоит узкий фильтр по `kubernetes.pod_labels.app`, а регулярка бьёт только по нужному тексту, не по всему потоку. Дорогой запрос не проскользнёт мимо ревью, а source of truth остаётся один: что в файле, то и исполняет `vmalert`. Grafana при этом остаётся читающим клиентом VictoriaLogs — datasource подключён, и логи можно исследовать в Explore, но создавать и править алерты через UI нельзя.
